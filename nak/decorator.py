...
def parser_config(*args, **kwargs):
    ...
    def _decorator(func):
        ...
        def _wrapper(self, parser, **func_kwargs):
            ...
            self.gateway.user_email = self.config.user_email
            self.gateway.password = self.config.password
            self.gateway.client_id = self.config.client_id
...
def check_error(error_format='{error_default} -> {error_msg}', response_json=True, **kwargs):
    ...
    def _decorator(func):
        ...
        def _wrapper(self, *func_args, **func_kwargs):
            ...
            error_default = f'{func.__name__.capitalize().replace("_", " ")} failed.'
            error_msg = ""
